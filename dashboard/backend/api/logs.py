...
async def get_logs(
    ...
    """Retrieve structured logs from ClickHouse with hybrid NoSQL fallback and full explainability metadata"""
    if ch.connected:
        return ch.get_logs(
            limit=limit,
            page=page,
...
async def fetch_recent_logs(
    ...
    """Fetch recent logs for Dashboard table view"""
    if ch.connected:
        res = ch.get_logs(limit=limit)
        if res.get("logs"):
